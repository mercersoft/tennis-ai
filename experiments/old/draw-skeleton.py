...
def extract_keypoints_for_avatar(keypoints) -> List[Dict[str, Any]]:
    ...
    Maps YOLO keypoints to COCO format (17 keypoints) for proper bone mapping.
    ...
    if len(keypoints_data) > 0:
        ...
        # Initialize with 17 keypoints (COCO format)
        for _ in range(17):
        ...
        # Map YOLO keypoints to COCO format
        for i, kpt in enumerate(person_kpts):
            x, y, conf = kpt
            if conf >= CONFIDENCE_THRESHOLD:
                # Map YOLO indices to COCO indices
                coco_index = i  # Default mapping
                if i < 17:  # Only map if within COCO range
                    avatar_keypoints[coco_index] = {
                        "x": float(x),
                        "y": float(y),
                        "confidence": float(conf)
                    }
